...
function initiateServer() {
	fs.readFile('./storage/ServerParams.json')
		...
		.then((serverParams) => {
			...
			console.log(`Server running at http://${serverParams.host}:${serverParams.port}/`);
			...
			app.get('/send', (req, res) => {
				getNextMessage()
					.then((message) => {
						mailer.sendEmail(message.destination, message.subject, message.data);
...
function getNextMessage() {
	return fs.readFile('./storage/NextMessage.json')
		.then((data) => {return JSON.parse(data);})
		.then((message) => {
			return message;	
		})
...
function insertMessage(message) {
	...
	return fs.writeFile("./storage/NextMessage.json", JSON.stringify(message))
		.then(() => { return 'Wrote new message'; })
		.catch((err) => { throw err; });
